...
if ($method === 'GET') {
	...
	$params = [];
	if ($q !== '') {
		$where[] = '(name LIKE :q OR username LIKE :q OR email LIKE :q OR location LIKE :q)';
		$params[':q'] = "%$q%";
	}
	...
	$offset = ($page - 1) * $per;
	$stmt = $pdo->prepare("SELECT id, name, username, email, age, location, bio, created_at FROM users $whereSql ORDER BY id DESC LIMIT :limit OFFSET :offset");
