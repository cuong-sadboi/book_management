(() => {
  const API = 'api/publishers.php';
  const tbody = document.getElementById('publisherTbody');
  const pagination = document.getElementById('publisherPagination');
  const searchInput = document.getElementById('publisherSearch');
  const perPageSelect = document.getElementById('publisherPerPage');
  ...
  function renderRows(rows) {
    ...
    tbody.innerHTML = rows.map((pub) => `
      <tr data-id="${pub.id ?? ''}">
        ...
        <td class="text-truncate" style="max-width: 260px;">${escapeHtml(pub.website ?? '')}</td>
        <td>
          <div class="d-flex gap-1">
            <button class="btn-edit btn btn-sm btn-outline-primary d-inline-flex align-items-center justify-content-center" data-id="${pub.id ?? ''}" title="Edit" aria-label="Edit">
            ...
            <a href="details-publisher.html?id=${encodeURIComponent(pub.id)}" class="btn-view btn btn-sm btn-outline-info d-inline-flex align-items-center justify-content-center" data-id="${pub.id ?? ''}" title="View Details" aria-label="View Details">
  ...
  btnDeleteSelected?.addEventListener('click', () => {
    ...
    if (!confirm(`Delete ${ids.length} selected publisher(s)?`)) return;
  ...
  tbody.addEventListener('click', (event) => {
    ...
    if (editBtn?.dataset.id) {
      ...
      window.location.href = `update-publisher.html?id=${editBtn.dataset.id}`;
      return;
    }
  });
