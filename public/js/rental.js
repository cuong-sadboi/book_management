    ...
    function renderRows(rows) {
        ...
        tbody.innerHTML = rows.map((r) => `
            <tr data-id="${r.id ?? ''}">
                ...
                <td>${escapeHtml(r.book_titles ?? '')} </td>
                ...
                <td>
                    <div class="d-flex flex-column gap-1">
                        ${r.status === 'active' || r.status === 'overdue' ? `<button class="btn-return btn btn-sm btn-success" data-id="${r.id ?? ''}" title="Return Book">Return</button>` : ''}
                        ${r.status !== 'returned' ? `<button class="btn-edit btn btn-sm btn-outline-primary" data-id="${r.id ?? ''}" title="Edit">
                        ...
                        <button class="btn-delete btn btn-sm btn-outline-danger" data-id="${r.id ?? ''}" title="Delete">
